...
bimap<Symbol, string> symbol_str_map = {
    ...
    {CONDITIONAL_ASSIGN, "?="}, {LEFT_CURLY, "{"}, {RIGHT_CURLY, "}"}, {STR_MATCH, "~"}, {BOOL, "BOOL"}, {ID, "ID"}, {NUM, "NUM"}, {STRING, "STRING"}, {AND, "and"}, {BREAK, "break"}, 
    {CONTINUE, "continue"}, {ELSE, "else"}, {IF, "if"}, {NOT, "not"}, {OR, "or"}, {REPEAT, "repeat"}, {TIMES, "times"}, {WHILE, "while"}, 
};
